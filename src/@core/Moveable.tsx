...
export type WillChangePositionEvent = PubSubEvent<
	"will-change-position",
	Position
>;
export type DidMoveEvent = PubSubEvent<"did-move", Position>;
export type DidChangePositionEvent = PubSubEvent<
	"did-change-position",
	Position
>;
...
export default function Moveable({ isStatic = false }: Props) {
	...
	const api = useComponentRegistry<MoveableRef>("Moveable", {
		...
		async move(targetPosition, type = "move") {
			...
			nodeRef.current && anime.remove(nodeRef.current.position);

			await anime({
				targets: nodeRef.current ? nodeRef.current.position : undefined,
				x: [fromX, toX],
				y: [fromY, toY],
	...
	useEffect(() => {
		...
		return () => (node ? anime.remove(node.position) : undefined);
	}, [nodeRef]);
